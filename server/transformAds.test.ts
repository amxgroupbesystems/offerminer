...
import { validateLowTicketLandingPage } from './landingPageClassifier';
...
function rawAd(id:string,overrides:Record<string,unknown>={}){
  return {
    ...
    snapshot:{
      body:{text:'Kit de atividades pedagógicas em PDF por apenas R$ 10. Acesso imediato e garantia.'},
...
test('mantém somente anúncios ativos, recentes e com destino comercial externo',()=>{
  ...
  const offers=transformAdsToOffers([rawAd('ok'),inactive,old,social],'Educação',['kit atividades'],now);
...
test('rejeita rifa física mesmo quando o texto contém preço e kit',()=>{
  ...
  assert.equal(transformAdsToOffers([raffle],'Educação',['kit'],now).length,0);
});

test('oculta oferta antiga ou indesejada já armazenada',()=>{
  const valid=transformAdsToOffers([rawAd('stored')],'Educação',['kit atividades'],now)[0];
...
test('deduplica a mesma mídia e copy e contabiliza anúncios que usam o criativo',()=>{
  const first=rawAd('one');
  const second=rawAd('two');
  const offers=transformAdsToOffers([first,second],'Educação',['kit atividades'],now);
...
test('valida estrutura de página low ticket e rejeita catálogo genérico',async()=>{
  const offer=transformAdsToOffers([rawAd('landing')],'Educação',['kit atividades'],now)[0];
  const originalFetch=globalThis.fetch;
  try{
    globalThis.fetch=async()=>new Response('<html><body><h1>Kit de atividades em PDF</h1><p>O que você vai receber</p><p>Bônus exclusivo</p><p>Garantia de 7 dias</p><strong>R$ 10,00</strong><a href="/checkout">Quero receber</a></body></html>',{status:200,headers:{'content-type':'text/html'}});
    assert.equal((await validateLowTicketLandingPage(offer)).accepted,true);
    globalThis.fetch=async()=>new Response('<html><body>Catálogo de produtos Filtrar produtos Ordene por Frete grátis Estoque Meus pedidos</body></html>',{status:200,headers:{'content-type':'text/html'}});
